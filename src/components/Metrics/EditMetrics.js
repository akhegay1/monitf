...
function EditMetric(props) {
  ...
  let [metricErr, setMetricErr] = useState("");

  const fetchMetric = async () => {
    console.log("start fetchMetric");

    let url = `${baseurl}metric?id=${metricstate.Id}`;
  ...
  async function saveClick() {
    console.log("saveClick metricstate.Password", metricstate.Password);
    ...
    if (oper === "edit") {
      ...
      await axios
        ...
        .then((response) => {
          ...
          vappcontext.pSetState("metric", "metric", "list");
        })
        .catch((error) => {
  ...
  return (
    <Fragment>
      ...
      <div className={cn(a.flexcontainer, a.flexcontainerend)}>
        ...
        <button
          className={cn(a.btn, a.btnoper)}
          onClick={() => vappcontext.pSetState("metric", "metric", "list")}
        >
          Cancel
        </button>
      </div>
