...
function Metrics() {
  ...
  let [metricserr, setMetricserr] = useState("");
  const [metricsPn, setMetricsPn] = useState(0);
  const [metricsRp, setMetricsRp] = useState(0);
  ...
  let rp = vappcontext.vMetricsRp;
  let pn = vappcontext.vMetricsPn;
  ...
  let apimetricerr;
  if (typeof vappcontext.vMetrics[0] != "undefined") {
    apimetricerr = vappcontext.vMetrics[0].err;
  }
    ...
    return (
      <div>
          ...
          <tbody>
            {vappcontext.vMetrics.map((item) => (
              <tr
                ...
                onDoubleClick={() =>
                  vappcontext.pSetState("metric", "metric", "edit")
                }
              >
                <td> {item.Id}</td>
                <td> {item.Hostname}</td>
        ...
        <CRUDbtns
          ...
          pSetParentRecs={(hosts) => vappcontext.pSetMetrics(hosts)}
        ...
        <Paging
          ...
          pSetParentPn={(pn) => setMetricsPn(pn)}
          pSetParentRp={(rp) => setMetricsRp(rp)}
        />
      </div>
    );
  }
